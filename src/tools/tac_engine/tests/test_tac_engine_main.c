...
static int run_specific_tests(const char* category) {
    ...
    } else if (strcmp(category, "working") == 0) {
        printf("Running only Working Tests...\n");
        run_tac_engine_working_tests();
    } else {
        printf("Unknown test category: %s\n", category);
        printf("Valid categories: lifecycle, execution, debugging, edge, stress, working\n");
        return 1;
    }
...
static int run_all_tests(void) {
    ...
    // Run working tests first to verify basic functionality
    run_tac_engine_working_tests();
